...
if [[ -z "$ACCOUNT_ID" || -z "$ROLE_NAME" || -z "$PROJECT_ID" || -z "$INTEGRATION_ID" ]]; then
  echo "❌ Missing required parameters. Please provide: --account_id, --role_name, --project_id, --integration_id"
  exit 1
fi
